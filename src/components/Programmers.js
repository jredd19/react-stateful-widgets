...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeFeature ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          feature.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setActiveFeature(dev.id)}>Feature</button>
            </div>
          )
        }
      </div>
